  ...
  for (const f of files) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      if (i === 1) {
        ...
        if (
          parent?.children &&
          !parent.children.some((c) => c.path === dirPath)
        ) {
...
export function FileTree({
  ...
  const staged = files.filter(
    (f) => f.flag === "staged" || f.flag === "both",
  );
  const unstaged = files.filter(
    (f) =>
      f.flag === "unstaged" || f.flag === "untracked" || f.flag === "both",
  );
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Summary bar */}
      <div className="flex h-8 shrink-0 items-center gap-1.5 border-b border-border bg-card px-2 text-[11px]">
      ...
      <div className="relative shrink-0 border-b border-border/50 px-1.5 py-1">
        ...
        <Input
          ...
          className="h-6 border-0 bg-transparent pl-6 pr-6 text-[11px] shadow-none outline-none focus-visible:ring-0"
        />
        {fileFilter && (
          <Button
            variant="ghost"
...
function TreeNodeRow({
  ...
  if (node.kind === "dir") {
    ...
    return (
      <>
        <div
          className={cn(
            "flex items-center gap-1 border-b border-border/20 px-1 py-0.5 text-[11px] hover:bg-accent/40",
          )}
        ...
        >
          ...
          <span
            className="ml-auto shrink-0 cursor-pointer text-[10px] tabular-nums text-muted-foreground"
            onClick={(e) => {
              e.stopPropagation();
              onToggleDir(node.path);
            }}
  ...
  return (
    <div
      className={cn(
        "flex cursor-pointer items-center gap-1.5 border-b border-border/20 py-0.5 pr-1 text-xs",
        "hover:bg-accent/40",
      ...
      {f.flag !== "unstaged" && f.flag !== "untracked" && (
        <span
          className={cn(
            ...
            f.flag === "staged"
              ? "bg-[hsl(var(--diff-added-bg))]/30 text-[hsl(var(--diff-added-fg))]"
              : f.flag === "both"
